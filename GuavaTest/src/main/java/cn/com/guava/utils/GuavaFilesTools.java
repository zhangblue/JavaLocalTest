...
public class GuavaFilesTools {
  ...
  public LargeLineProcessor readLargeFile(String strPath, long beginLineNum, int readSize) {
    ...
    try {
      CharSource charSource = Files.asCharSource(new File(strPath), Charsets.UTF_8);
      List<String> list = charSource.readLines();
  ...
  public void appendContentToFile(List<String> strContent, File file) {
    for (String strLine : strContent) {
      try {
        Files.append(strLine, file, Charset.defaultCharset());
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
